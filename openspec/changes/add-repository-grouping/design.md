...
- Repository mode derives sorted repository groups from organization PR data. Node keys use `repo:<owner>/<repository>` and cannot collide with PR keys.
- Repository nodes are focusable. PR actions are disabled naturally when no PR is focused.
- `j/k`, arrows, `gg/G` traverse visible repository nodes and PR leaves. `h/l`, left/right, `o`, `O`, and Enter provide tree collapse/expand behavior.
...
- Collapse state is stored by stable repository key and survives refresh and view toggling. Organization collapse remains independent.
- Repository nodes with zero visible PRs after draft filtering are omitted.

## Risks / Trade-offs
- Focus can refer to a PR or repository node. A collision-proof key prefix and small pure visibility helpers contain this complexity.
