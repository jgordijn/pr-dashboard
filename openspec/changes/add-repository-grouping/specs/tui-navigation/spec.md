...
- **THEN** `j/k`, down/up, `gg`, and `G` SHALL traverse visible organization nodes, repository nodes, and PR leaves
...
- **AND** pressing left or `h` on an expanded repository node SHALL collapse it
- **AND** pressing left or `h` on a collapsed repository node SHALL focus its organization parent
...
#### Scenario: Toggle all tree nodes
- **WHEN** `O` is pressed in repository mode
- **THEN** all organization and repository nodes SHALL expand if any is collapsed, otherwise all SHALL collapse

#### Scenario: Activate focused item
- **WHEN** Enter is pressed on an organization or repository node
...
- **WHEN** an organization or repository node is focused
